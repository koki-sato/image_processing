 ...
 *
 * コンパイルの方法
 * gcc -Wall -O1 my_edge_detection.c -o my_edge_detection -ljpeg
 * (for OpenMP)
 * gcc -Wall -O1 -fopenmp my_edge_detection.c -o my_edge_detection -ljpeg
 *
 * 実行方法
 * ./my_edge_detection (入力ファイル名) (出力ファイル名)
 *
 */
 ...
 * 画像の転置を行う関数
 **********************************************************************/
void edge_detection_serial(JSAMPARRAY inbuf, JSAMPARRAY outbuf, int width, int height) {
    int i, j, k, l;
    double h, v;
    unsigned char y;
    ...
    for (i = 1; i < height - 1; i++) {
        for (j = 1; j < width - 1; j++) {
            ...
            for (k = 0; k < 3; k++) {
                for (l = 0; l < 3; l++) {
                    h += inbuf[i+k-1][(j+l-1)*3] * Ch[k][l];
                    v += inbuf[i+k-1][(j+l-1)*3] * Cv[k][l];
                }
            }
            y = (unsigned char)sqrt(h * h + v * v);
            if (y < threshold) {
                outbuf[i][j*3] = outbuf[i][j*3+1] = outbuf[i][j*3+2] = 0;
            } else {
                outbuf[i][j*3] = outbuf[i][j*3+1] = outbuf[i][j*3+2] = y;
            }
        }
 ...
 **********************************************************************/
#ifdef _OPENMP
void edge_detection_parallel(JSAMPARRAY inbuf, JSAMPARRAY outbuf, int width, int height) {
    int i, j, k, l;
    double h, v;
    unsigned char y, y1, y2, y3, y4;
    double threshold = 70.0;
    int Ch[3][3] = {
        { 1, 0, -1 },
        { 2, 0, -2 },
...
#pragma omp parallel for private(j, h, v, k, l, y)
    for (i = 1; i < height - 1; i++) {
        for (j = 1; j < width - 1; j++) {
            h = v = 0;
            for (k = 0; k < 3; k++) {
...
#ifdef _OPENMP
    edge_detection_parallel(in_img->buf, out_img->buf, width, height);
#else
    edge_detection_serial(in_img->buf, out_img->buf, width, height);
#endif
